      ...
      <div className="min-h-screen">
        <div className="container mx-auto px-4 pt-6 max-w-3xl">
          <div className="glass p-5 mb-4">
            ...
            <div className="h-10 w-48 bg-white/10 rounded-lg animate-pulse mb-5" />
            <div className="h-12 w-full bg-white/10 rounded-2xl animate-pulse" />
          </div>
          <div className="glass p-4 mb-4">
            <div className="h-5 w-20 bg-white/10 rounded animate-pulse mb-4" />
            {[1, 2, 3, 4, 5].map(i => (
        ...
        <div className="glass p-5 mb-4">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            {!username && (
              <Link
                href="/login"
                className="glass-btn text-xs px-3.5 py-1.5 rounded-full"
              >
                登录 / 注册
              </Link>
            )}
          ...
          {username ? (
            <div className="text-white mb-5">
              <div className="text-xs text-white/50 mb-1.5">总资产（元）</div>
              <div className="text-4xl font-bold font-mono tracking-tight mb-4 text-gradient">
              ...
              <div className="flex gap-6 text-xs">
                <div>
                  <div className="text-white/50 mb-0.5">持仓</div>
                  <div className="font-mono font-semibold text-sm text-white/95">
                    {stats.totalHolding.toLocaleString("zh-CN", { minimumFractionDigits: 2 })}
                  </div>
                </div>
                <div>
                  <div className="text-white/50 mb-0.5">在途</div>
                  <div className="font-mono font-semibold text-sm text-white/95">
                    {stats.totalInTransit.toLocaleString("zh-CN", { minimumFractionDigits: 2 })}
                  </div>
                </div>
                <div>
                  <div className="text-white/50 mb-0.5">今日收益</div>
            ...
            <div className="text-white mb-5">
              <div className="text-xs text-white/50 mb-1">理财净值 · 数据每日更新</div>
              <div className="text-xl font-bold tracking-tight">登录后查看我的持仓</div>
            </div>
          )}
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white/[0.08] backdrop-blur-xl border border-white/15
                         rounded-2xl pl-11 pr-4 py-3 text-sm text-white
                         placeholder-white/40
                         transition-all duration-300 ease-out
                         focus:outline-none focus:ring-2 focus:ring-purple-400/40
                         focus:bg-white/[0.12] focus:border-white/30"
            />
          </div>
        </div>
      </div>
              ...
              return (
                <button
                  ...
                  className={`py-2.5 rounded-xl text-xs font-medium
                              ...
                              ${
                    isActive
                      ? "glass-btn-primary scale-[1.03]"
                      : "glass-soft text-white/60 hover:bg-white/[0.10] hover:text-white"
                  }`}
                >
                  <span>{t.icon}</span>
                  <span>{t.label}</span>
          ...
          <div className="glass overflow-hidden">
            <div className="min-h-[260px]">
              ...
              {activeTab === "profit" && (
                <div>
                  {topProfit.length === 0 ? (
                    <div className="py-12 text-center text-white/40 text-xs">暂无数据</div>
                  ) : (
                      ...
                      {topProfit.map((p, i) => (
                        ...
                        >
                          <span className={`w-6 h-6 rounded-lg flex items-center justify-center
                                            text-[11px] font-bold mr-3 flex-shrink-0 ${
                            i === 0
                              ? "bg-gradient-to-br from-rose-500 to-pink-600 text-white shadow-lg shadow-rose-500/40"
                              : i === 1
                              ? "bg-gradient-to-br from-orange-400 to-amber-500 text-white shadow-lg shadow-orange-500/30"
                              : i === 2
                              ? "bg-gradient-to-br from-yellow-400 to-amber-400 text-white shadow-lg shadow-yellow-500/30"
                              : "bg-white/10 text-white/60"
                          }`}>{i + 1}</span>
                          <div className="flex-1 min-w-0">
                            <div className="text-sm text-white/95 truncate">{p.name}</div>
                            <div className="text-[10px] text-white/50 mt-0.5">{p.bank}</div>
                          </div>
                          <div className="text-sm font-bold font-mono ml-2 flex-shrink-0 text-gradient-cyan">
                  ...
                  <Link
                    href="/discover?tab=profit"
                    className="block text-center py-3 text-xs text-white/70
              ...
              {activeTab === "hot" && (
                <div>
                  {hotSearches.length === 0 ? (
                    <div className="py-12 text-center text-white/40 text-xs">暂无搜索记录</div>
                  ) : (
                    <div className="divide-y divide-white/5">
                      {hotSearches.map((h, i) => (
                        <Link
                        ...
                        >
                          ...
                          <span className="flex-1 text-sm text-white/95 truncate">{h.keyword}</span>
                          <span className="text-xs text-white/50 mr-2">{h.count} 次</span>
                    ...
                    <div className="divide-y divide-white/5">
                      {newProducts.map((p) => (
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            <div className="text-sm text-white/95 truncate">{p.name}</div>
                            <div className="text-[10px] text-white/50 mt-0.5">{p.bank} · {p.nav_date}</div>
                          </div>
                          <div className="text-sm font-mono text-white/70 ml-2 flex-shrink-0">
        ...
        <div className="glass-soft rounded-2xl p-4 mb-6">
          <div className="flex items-start gap-2.5">
            ...
            <div className="text-[11px] text-white/50 leading-relaxed">
              <div className="font-medium text-white/70 mb-1">免责声明</div>
